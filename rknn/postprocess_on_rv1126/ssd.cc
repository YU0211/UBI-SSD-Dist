...
#define LABEL_NALE_TXT_PATH "./model/UBI_labels.txt" // contains the labels:[BACKGROUND, vehicle, rider, pedestrian]
...
int filterValidResult(float *outputClasses, int (*output)[NUM_RESULTS], int numClasses, float *props)
{
    ...
    for (int i = 0; i < NUM_RESULTS; ++i)
    {
        ...
        if (topClassScore >= min_score)
        {
            ...
            props[validCount] = outputClasses[i * numClasses + topClassScoreIndex];
            ++validCount;
        }
    }
...
int postProcessSSD(float *scores, float *boxes, int width, int heigh, detect_result_group_t *group)
{
    ...
    int validCount = filterValidResult(scores, output, NUM_CLASS, props);
    ...
    for (int i = 0; i < validCount; ++i)
    {
        ...
        int x1 = (int)(boxes[n * 4 + 0] * width);
        int y1 = (int)(boxes[n * 4 + 1] * heigh);
        int x2 = (int)(boxes[n * 4 + 2] * width);
        int y2 = (int)(boxes[n * 4 + 3] * heigh);
